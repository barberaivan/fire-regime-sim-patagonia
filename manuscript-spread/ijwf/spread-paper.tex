...
% [Claude, let's cut words by merging these 2 sections about elongation and geometry,
% renamed as "Problems with the spread geometry". Try to keep all most relevant ideas,
% but reduce length.]
\subsection*{The elongation ceiling}

The clearest mismatch in both validation exercises is shape, and it shows a structural
problem. In the automaton the spread \emph{rate} is isotropic:
...
so the downwind front cannot easily outrun the flanks. When the wind term is strong 
it drives spread probability towards 1 downwind and 0 upwind, but leaves the two
neighbours perpendicular to the wind exactly where they would be with no wind at all. The diagonal neighbours 45$^\circ$ off the wind axis still take
71\,\% of the wind term, so their spread probability stays high and the flanks
advance one cell per step as well. 
Simulated fires in flat, homogeneous fuel then make a funnel widening downwind,
never the narrow, cigar-shaped run of a wind-driven fire.
The one regime in which the automaton does produce
...
But that requires low spread probabilities, which rarely produce large fires. The model
can therefore make fires that are elongated or large, but not both.
This may also explain the size mismatch: given the same step budget ($\kappa$),
a fire that spreads compactly burns far more area than one that spreads as an ellipse.

Further analysis shows that the shape mismatch is mostly carried by the fires that
stopped because they used up their step budget, which confines them to a square around
their ignition cell (Section~5 of the Supplementary material). These fires are clipped
along the downwind axis first, so what survives is rounder and turned across the wind,
giving a larger deviation from the wind direction. Fires that ended because propagation
failed at every edge cell are closer to the mapped record in compactness and in wind
alignment. This suggests that the automaton's geometry may do better in a regime of
lower spread probability (as in \citealt{morales_stochastic_2015}), but such a regime
makes fire size harder to control and leaves many unburned islands within a fire, which
would distort the simulation of post-fire vegetation transitions: unrealistic unburned
patches would overestimate the regeneration capacity of seeder species.

\subsection*{Towards a better geometry}
...
fire is set by the grid rather than by the wind. 
Elliptical growth models treat every point on the front as the source of an
...
thousands of simulations a regime simulator requires. What makes them
...
available. With better data the unit could be shrunk to a day, half a day or six hours,
and datasets of fire progression that did not exist when this record was compiled now
make that possible. One is global and coarse: GlobFire delimits individual fire events
and their daily burned area worldwide from the 500\,m MODIS product
\citep{artes_global_2019}, and the same approach has been applied across the United
States \citep{balch_fired_2020}. The others are regional and deliberate efforts:
half-daily perimeters and active fronts tracked from VIIRS detections across California
\citep{chen_california_2022}, or PT-FireSprd, which reconstructed the progression of 80
Portuguese fires larger than 100\,ha from satellite, airborne and ground observations
together with the records of the fire crews, and reports a rate of spread for each
burning period \citep{benali_portuguese_2023}.
%[Claude, make this shorter. Cite the global one separately and mention that regional
% efforts get more detail, but they take more effort (and cite all examples). The important
% is mentioning that small fires are left alone.]
...
(Table~\ref{tab:shape}), so a model fitted to progression data alone would describe
large fires and inherit their size distribution. Informing different parameters with
different datasets, as we did through the area regression, is what avoids that: the
spread functions from the large fires with progression data, and the few parameters that
govern fire size and extinction, like our $\kappa$, from the final perimeters of the
whole record, which Landsat and Sentinel make increasingly available. Ideally the daily
advance of small fires would be mapped as well, with imagery of high spatial and
temporal resolution: PlanetScope delivers near-daily 3\,m images and resolves burns too
small and too fragmented for coarser sensors \citep{martins_deep_2022}.
% [Claude, This second paragraph can also be shorter, but keep that key citation and planet example]

\subsection*{Scope of the hierarchical structure}
...
We believe that four of its strategies are useful for developing the spread component
...
the old ones. All of them could be adopted with a better spread engine.
