...
Fire spread models are a core component of for fire simulation,
...
Despite fuel model maps can be assembled at large scales by matching 
vegetation maps to the native fuel models of each model, 
these adaptation of simulators to new systems still requires 
detailed fire behaviour data to tune parameters in the best case, 
or at least to validate them.
Hence, a reasonable solution is to develop simpler models that can
be run, fitted and validated using the available data, which may be 
limited to simply fire polygons (e.g., \citealt{morales_stochastic_2015}). 
...
a fire event to inform combat, but they may be useful to simulate fire regimes 
\citep{parisien2020commentary}.
% [Claude, is this Parisien commentary good here?]
...
a cellular automaton in which the probability of spread between
...
fire record, where only final fire polygons are available \citep{barbera_patterns}. 
Rate of spread is never measured, daily progression is not mapped, 
...
% Acá falta resaltar la estructura jerárquica, que permite simular variabilidad 
% que el modelo ignora por ser tan simple. Esa variabilidad se estima de los datos
% y luego puede simularse. 
% A su vez, este enfoque require el punto de ignición; las bases de datos tienen
% un sesgo hacia fuegos grandes, entonces usamos distintos sets de datos para 
% informar distintos sets de parámetros. Esto permite que el modelo también sea capaz
% de simular fuegos pequeños, lo cual es requerido para simular el régimen de 
% incendios de forma correcta. 
