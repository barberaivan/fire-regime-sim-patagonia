...
Further analysis (Section~5 of the Supplementary material) shows that the shape mismatch is
carried by the fires the step budget cut short: confined to a square around their ignition
cell, they are clipped along the downwind axis first, so what survives is rounder and turned
across the wind. Fires that ended instead because propagation failed at every edge cell are
close to the mapped record in compactness and in wind alignment. The automaton's geometry may
therefore be less poor than the headline comparison makes it look. What it needs is a regime
of lower spread probability, which is also the regime in which size becomes hard to control.
...
FWI (Fig.~S3), the fit to the 57 fires with a known ignition point, as spatial
overlap (Fig.~S4) and as the ratio of simulated to observed size (Fig.~S5), and the shape
of the simulated fires split by what brought each of them to a stop (Table~S2, Fig.~S6).
