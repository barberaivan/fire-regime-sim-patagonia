...
built where the inputs the physical models need do not exist. Since their purpose is to characterize a fire regime rather than to
...
\item \textbf{Atmospheric conditions}, summarized by the Fire Weather Index (FWI;
...
\citet{vitolo_era5-based_2020}. The index was standardized per pixel, so that it
...
parameters we summarized them into a \textbf{vegetation flammability index} (VFI) and a
...
only a rasterized perimeter, so the observed reference is every mapped fire in the
...
vegetation types at a common topography while marginalizing over the landscape, on
