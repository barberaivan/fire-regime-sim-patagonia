...
Bayesian Computation (ABC; \citealt{beaumont2010approximate, sisson2018handbook}). 
...
After running 148\,649 fires and discarding those below 10\,ha 
(same threshold as in the mapped record)
we obtained 64\,836 simulated fires to compare against the 235 observed ones. 
...
% [2026-09-07, Claude] This paragraph used to end: "under simulated random effects it
% overshot much further (0.686, only 19 % below)". That reading was backwards. In
% DHARMa a residual ABOVE 0.5 means the observation sits high in its predictive
% distribution, i.e. the model simulates too LITTLE; the convention is stated in
% spread/figure_dharma_metrics.R line 11 ("below it = the model simulates too much,
% above it = too little"). The numbers were right, the words were not.
% The apparent clash with the size quotient of 1.51 in the previous paragraph is real
% but explainable, and the paragraph now says so: that quotient is a posterior MEAN of
% simulated/observed per fire (spread/figure_focal_fit.R: "The quotient is formed per
% simulation and then summarised"), inflated by the long upper tail, whereas the
% residual is a MEDIAN-based rank. Under simulated random effects the per-fire
% predictive distribution is so wide that its median falls short of the observed fire
% while its mean exceeds it, so both statistics are correct at once.
% The same misreading was in docs/spread.md and was corrected there too. Neither figure
% was re-run to check the numbers: data/ and files/ are not symlinked on that machine.
...
a fire's observed value falls among its own simulations: 0.5 is calibration, lower means
the model burns too much, higher too little. For burned area under fitted random
effects the observed fires sat low (mean residual 0.373, 65\,\% of them below their
simulated median), so the model burns too much. Under simulated random
effects the residuals moved the other way (0.686, 19\,\% below), because that
...
% [Claude, this figure above should include the fitted steps for the fires with unknown
% ignition point. Change it and update the caption to mention that.
% They should have another color, in the same scale ideally.]
