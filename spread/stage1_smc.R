...
data_dir <- file.path("data", "focal_fires", "landscapes")
...
# saveRDS(size_data, file.path("data", "focal_fires", "fire_size_data.rds"))
size_data <- readRDS(file.path("data", "focal_fires", "fire_size_data.rds"))

# rownames(size_data) <- 1:nrow(size_data)
# write.csv(size_data, file.path("data", "focal_fires", "fire_size_data.csv"))
  ...
  while(go_on) {
    ...
    if (go_on) {
      if (write_file) { # save partial result
        ...
        fdir <- file.path(
          "files", "posterior_samples_stage1", fname
        )
        saveRDS(res, fdir)
      }
    } else {
      ...
      # remove partials
      dirr <- file.path(
        "files", "posterior_samples_stage1"
      )
      fn <- list.files(dirr)
...
# TODO(migration): disabled — see docs/migration.md #5. In the OLD repo, this compared the
# SMC output against files/posterior_samples_stage1/ (bare name), the output of the LEGACY
# importance-sampling script (sampling_fire_wise_posteriors_IMPORTANCE.R, "not used" per
# INVENTORY.md §6). This migration renames the SMC folder to that same bare
# files/posterior_samples_stage1/ (Reference B), so re-enabling this as-is would silently
# compare the SMC output against itself. The legacy importance-sampling data was not migrated
# (non-canonical). Re-derive from the archived old repo if this comparison is needed again.
...
# TODO(migration): disabled for the same reason as above — see docs/migration.md #5.
