...
paste("input_direction            =", wind_direction)
  ...
  # Run WindNinja
  system(paste0("WindNinja_cli --config_file=", file.path(windninja_dir, "spread_config_file.cfg")))
}

# delete configuration file
unlink(file.path(windninja_dir, "spread_config_file.cfg"))
...
mdetrend <- readRDS(file.path("data", "flammability_indices",
...
# Make landscapes ---------------------------------------------------------
# TODO(refactor): this loop over focal fires (and its PNNH counterpart further
# below) should become a function that builds any landscape, not a hard-coded
# loop — see docs/migration.md #5 (deferred; not part of this migration).
...
for(i in 1:length(lands)) {
  print(i)
  saveRDS(lands[[i]], file.path("data", "focal_fires",
                                "landscapes",
...
elev_path <- file.path("data", "pnnh_images", "pnnh_data_spread_elevation_30m.tif")
# create config file
...
# Run WindNinja
# system(paste0("WindNinja_cli --config_file=", file.path(windninja_dir, "spread_config_file_pnnh.cfg")))
