...
# Simulation-tile geometry ------------------------------------------------

#' Rectangular tiles covering the study area, for simulating new fires
#'
#' The R twin of the GEE script "Landscapes export for simulation (study area
#' tiles)" — keep the two in sync. The study area is cut into `k` latitudinal
#' pieces of equal latitudinal length; each piece is reduced to its bounding
#' box, buffered by `buffer`, and reduced to a bounding box again, so every tile
#' is a rectangle. Consecutive tiles therefore overlap by ~2 * `buffer`, which
#' is the point: a fire ignited near a tile's edge still has room to spread.
#'
#' @return SpatVector of `k` rectangles in `crs`, with fields `tile`,
#'   `width_km`, `height_km`, `megapixels_30m`.
study_area_tiles <- function(study_area, k = 4, buffer = 10000,
                             crs = "EPSG:5343") {

  sa <- project(study_area, crs)
  e <- ext(sa)
  ybreaks <- seq(e$ymin, e$ymax, length.out = k + 1)

  rects <- lapply(1:k, function(i) {
    strip <- ext(e$xmin, e$xmax, ybreaks[i], ybreaks[i + 1])
    piece <- ext(crop(sa, strip))
    ext(piece$xmin - buffer, piece$xmax + buffer,
        piece$ymin - buffer, piece$ymax + buffer)
  })

  width_km <- sapply(rects, function(r) (r$xmax - r$xmin) / 1000)
  height_km <- sapply(rects, function(r) (r$ymax - r$ymin) / 1000)

  tiles <- vect(lapply(rects, as.polygons, crs = crs))
  tiles$tile <- 1:k
  tiles$width_km <- width_km
  tiles$height_km <- height_km
  tiles$megapixels_30m <- width_km * height_km * 1e6 / 900 / 1e6
